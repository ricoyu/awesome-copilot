...
public class ElasticMinAggregationBuilder extends AbstractAggregationBuilder implements ElasticAggregationBuilder {
	...
	private static final Logger log = LoggerFactory.getLogger(ElasticMinAggregationBuilder.class);
	...
	public Double get() {
		...
		try {
			...
			log.error("Min aggregation failed: {}", errorDetail, e);
		...
		return V8AggResultSupport.minResult(aggregations, name);
	}
}
