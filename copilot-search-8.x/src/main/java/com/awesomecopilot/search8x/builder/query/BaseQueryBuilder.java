...
import com.awesomecopilot.search8x.enums.SortOrder.SortOrderBuilder;
...
import com.awesomecopilot.search8x.vo.ElasticScroll;
...
public abstract class BaseQueryBuilder implements BoolQuery {
	...
	public long queryForCount() {
		...
		SearchResponse response = SearchRequestSupport.search(ElasticUtils.QUERY_CLIENT, indices, sourceBuilder);
	...
	public long delete() {
		try {
			...
			return com.awesomecopilot.search8x.support.SearchResponseBridge.deleteByQuery(
					ElasticUtils.QUERY_CLIENT, indices, builder().toString(), doRefresh);
		} catch (Exception e) {
			log.error("", e);
	...
	private SearchResponse searchResponse() {
		if (isNotBlank(scrollId)) {
			return SearchRequestSupport.search(ElasticUtils.QUERY_CLIENT, indices,
					new SearchRequestSupport.SearchOptions().scrollId(scrollId));
		}

		SearchRequestSupport.SearchOptions options = new SearchRequestSupport.SearchOptions()
				.query(builder())
				.trackTotalHits(true);
		if (scrollWindow != null) {
			// 将毫秒数转换为时间字符串格式
			long scrollMillis = timeUnit.toMillis(scrollWindow);
			String scrollTimeStr = scrollMillis + "ms";
			options.scroll(scrollTimeStr);
		}
		if (this instanceof Highlightable) {
			options.highlight(((Highlightable) this).toHighlightBuilder());
		}
		if (!highlightFields.isEmpty()) {
			...
			options.highlight(highlightBuilder);
		}
		if (storedFields != null) {
			options.storedFields(storedFields);
		}
		List<SortBuilder<?>> sorts = new ArrayList<>();
		sortOrders.forEach(sortOrder -> sorts.add(sortOrder.toSortBuilder()));
		options.sorts(sorts);
		...
		if (includeSource != null && includeSource.length > 0 || excludeSource != null && excludeSource.length > 0) {
			options.includeSource(includeSource).excludeSource(excludeSource);
		} else {
			options.fetchSource(fetchSource);
		}
		if (!scriptFields.isEmpty()) {
			for (Map.Entry<String, String> entry : scriptFields.entrySet()) {
				options.scriptField(entry.getKey(), new Script(entry.getValue()));
			}
		}

		SearchSourceBuilder preview = new SearchSourceBuilder();
		options.toConfigurer().configure(preview);
		if (log.isDebugEnabled()) {
			String json = preview.toString();
		...
		return SearchRequestSupport.search(ElasticUtils.QUERY_CLIENT, indices, options);
	}
