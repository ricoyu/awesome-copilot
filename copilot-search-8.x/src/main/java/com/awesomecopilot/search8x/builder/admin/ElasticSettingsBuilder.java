...
public class ElasticSettingsBuilder {
	...
	Map<String, Object> build() {
		...
		if (isNotBlank(indexRoutingAllocationKey) && isNotBlank(indexRoutingAllocationValue)) {
			settings.put("index.routing.allocation.require." + indexRoutingAllocationKey, indexRoutingAllocationValue);
		}
		if (blocksWrite != null) {
			settings.put("index.blocks.write", blocksWrite);
		}
		if (log.isDebugEnabled()) {
			log.debug("Settings:\n{}", new JSONObject(settings).toString(2));
		}
		return settings;
	}
