...
public class ElasticAvgAggregationBuilder extends AbstractAggregationBuilder implements ElasticAggregationBuilder {
	...
	private static final Logger log = LoggerFactory.getLogger(ElasticAvgAggregationBuilder.class);
	...
	public Double get() {
		...
		try {
			...
			log.error("Avg aggregation failed: {}", errorDetail, e);
		...
		return V8AggResultSupport.avgResult(aggregations, name);
	}
}
