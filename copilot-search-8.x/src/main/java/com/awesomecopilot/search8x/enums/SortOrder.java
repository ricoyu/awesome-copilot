...
public class SortOrder {
	...
	/**
	 * 为 SearchSourceBuilder 添加排序规则
	 */
	public void addTo(SearchSourceBuilder builder) {
		builder.sort(toSortBuilder());
	}

	public SortBuilder<?> toSortBuilder() {
		if (sortType == SortType.SCORE) {
			return SortBuilders.scoreSort().order(direction.toSortOrder());
		}
		return SortBuilders.fieldSort(field).order(direction.toSortOrder());
	}
