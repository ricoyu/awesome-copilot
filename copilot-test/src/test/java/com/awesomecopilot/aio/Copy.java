package com.awesomecopilot.aio;
...
public class Copy {
	...
	public static void main(String[] args) throws IOException {
		...
		copy(AsynchronousFileChannel.open(Paths.get(args[0])),
				AsynchronousFileChannel.open(Paths.get(args[1]), WRITE, TRUNCATE_EXISTING, CREATE));
		try {
			THDMAIN.join();
	...
	public static void copy(AsynchronousFileChannel chSrc, AsynchronousFileChannel chDest) {
		...
		class ReadCompletionHandler implements CompletionHandler<Integer, Integer> {
			...
			public void completed(Integer result, Integer pos) {
				...
				buffer.flip();
				chDest.write(buffer, pos, pos + result, new CompletionHandler<Integer, Integer>() {
					@Override
					public void completed(Integer result, Integer newPos) {
						buffer.compact();
						chSrc.read(buffer, newPos, newPos, ReadCompletionHandler.this);
					}
