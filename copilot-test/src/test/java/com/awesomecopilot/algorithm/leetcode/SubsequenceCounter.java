 ...
 * 不同的子序列
 * <p>
 * 这是一个经典的**动态规划（DP）**问题，目标是统计字符串 s 中有多少个子序列等于字符串 t。
 * <p>
 * 子序列不要求连续，但必须保持字符的相对顺序。
 * <p>
 ...
 * 定义动态规划数组：
 * <ul>
 *     <li/>我们定义 dp[i][j] 表示：字符串 s 的前 i 个字符中，包含多少个 t 的前 j 个字符构成的子序列。
 *     <li/>边界条件：如果 t 是空串，那么 s 的任意前缀都包含空串，故对所有 i，dp[i][0] = 1。
 * </ul>
 * 状态转移方程：
 * <ul>
 *     <ul>如果 s[i-1] == t[j-1]，有两种选择
 *        <li/>使用 s[i-1] 匹配 t[j-1] → 继承 dp[i-1][j-1]
 *        <li/>不使用 s[i-1] → 继承 dp[i-1][j]
 *        <li/>所以：dp[i][j] = dp[i-1][j-1] + dp[i-1][j]
 *     </ul>
 *     <li/>否则，只能不使用 s[i-1] → dp[i][j] = dp[i-1][j]
 * </ul>
