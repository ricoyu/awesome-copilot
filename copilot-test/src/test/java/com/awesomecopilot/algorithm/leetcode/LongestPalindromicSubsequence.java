 ...
 *     <li/>状态转移方程：如果 s[i] == s[j]，那么 dp[i][j] = dp[i+1][j-1] + 2，
 *          如果字符串两端的字符相等, 那么这两个字符一定可以构成最长回文子序列的一部分。
 *          此时, 整个区间[i,j]的最长回文子序列长度, 就等于去掉这两个字符后中间区间[i+1,j-1]的最长回文子序列长度, 再加上这两个匹配的字符（+2）。
