...
public class MessageFormatTest {
	...
	public void testFormatLong2() {
		...
		message =
				MessageFormat.format("This is a formatted {0, number,#.##} and {1, number,#.##} numbers", 25.7575, 75.2525);
		System.out.println(message);
		
		// Format a number as currency.
		message =
				MessageFormat.format("This is a formatted currency {0,number,currency} and {1,number,currency} numbers", 25.7575, 25.7575);
		System.out.println(message);
		
		// Format numbers in percentage.
		message =
				MessageFormat.format("This is a formatted percentage {0,number,percent} and {1,number,percent} numbers", 0.10, 0.75);
		System.out.println(message);
	}
	...
	public void testFormatTypes() {
		String pattern =
				"On {0, date}, {1} sent you {2, choice, 0#no messages|1#a message|2#two messages|2<{2, number, integer} messages}.";
	...
	public void testPerformance() {
		...
		System.out.println("MessageFormat.format 10万次花费" + (end - start) / 1000);
		start = System.currentTimeMillis();
		for (int i = 0; i < 1000000; i++) {
			String message2 = String.format("结算单号[{0}]", 123456L);
		}
		end = System.currentTimeMillis();
		System.out.println("String.format 10万次花费" + (end - start) / 1000);
	}
}
