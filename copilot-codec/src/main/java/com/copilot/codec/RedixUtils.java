...
public class RedixUtils {
    ...
    /**
     * 十六进制转二进制字符串, 比如0x0F, 输出 00001111
     *
     * @param hex
     * @return String
     */
    public static String hex2BinaryStr(String hex) {
        ...
        // 补足长度为16进制数字的四倍
        while (binaryString.length() < hex.length() * 4) {
    ...
    public static String ascii2Hex(String asciiString) {
        ...
        for (int i = 0; i < asciiString.length(); i++) {
            // 获取当前字符
            char ch = asciiString.charAt(i);

            // 将字符转换为16进制，使用Integer.toHexString()方法
            // 并将结果添加到StringBuilder中
            String hex = Integer.toHexString(ch);

            // 为了保证每个16进制数字都是两位数，如果转换结果是单个字符，则前面补零
            if (hex.length() == 1) {
                hexString.append('0');
            }
