...
import com.awesomecopilot.codec.exception.PrivateEncryptException;
import com.awesomecopilot.codec.exception.PublicDecryptException;
import com.awesomecopilot.codec.exception.PublicEncryptException;
import com.awesomecopilot.codec.exception.RsaPrivateKeyException;
import com.awesomecopilot.codec.exception.RsaPublicKeyException;
import com.awesomecopilot.codec.exception.RsaSignException;
...
public final class RsaUtils {
	 ...
	 * 只保留长度与 SHA-256 摘要前 8 位, 便于排查又不泄露内容。
	 */
	private static String sanitize(byte[] data) {
		if (data == null) {
			return "null";
		}
		return "len=" + data.length + ", sha256=" + HashUtils.sha256(data).substring(0, 8);
	...
	public static String publicEncrypt(String data) {
		try {
			...
			throw new PublicEncryptException("加密字符串[" + sanitize(data) + "]时遇到异常", e);
		}
	}
	...
	public static String publicDecrypt(String data) {
		...
		try {
			...
			throw new PublicDecryptException("解密字符串[" + sanitize(data) + "]时遇到异常", e);
		}
	}
	...
	public static String privateEncrypt(String data, String privateKeyStr) {
		...
		try {
			...
			throw new PrivateEncryptException("加密字符串[" + sanitize(data) + "]时遇到异常", e);
		}
	}
	...
	public static String sign(byte[] data) {
		...
		try {
			...
			throw new RsaSignException("签名字符串[" + sanitize(data) + "]时遇到异常", e);
		}
	}
	...
	public static boolean verify(byte[] data, String sign) {
		...
		try {
			...
			throw new RsaSignVerifyException("验签字符串[" + sanitize(data) + "]时遇到异常", e);
		}
	}
